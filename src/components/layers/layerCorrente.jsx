    ...
    const carregarGeoJSON = async () => {
      try {
        ...
        const [respVelocidade, respDirecao] = await Promise.all([
          fetch(`/data/${fonteDados}/corrente/velocidade/timestep_${timestep}.geojson`, { signal }),
          fetch(`/data/${fonteDados}/corrente/direcao/timestep_${timestep}.geojson`, { signal })
        ]);
...
setGeojsonDirecao(direcaoNormalizada);
  ...
  useEffect(() => {
    ...
    const carregarDataBase = async () => {
      try {
        const response = await fetch(`/data/${fonteDados}/vento/velocidade/timestep_000.geojson`);
