...
export default function LayerNivel({
  ...
  const [dataBaseTimeline, setDataBaseTimeline] = useState(null); // Base fixa para os marcadores

  // Limites dinâmicos iguais aos outros componentes
  ...
  // Effect do Timer (Play/Pause) corrigido com o limite dinâmico
  useEffect(() => {
    let interval = null;
    if (isPlaying) {
  ...
  // Effect para carregar os dados temporais de Nível (Corrigido com padStart)
  useEffect(() => {
    ...
    // Transforma o timeStep em formato de 3 dígitos (ex: 005 em vez de 5)
    const timestepFormatado = String(timeStep).padStart(3, "0");
    const filename = `/data/${fonteDados}/nivel/geojson/timestep_${timestepFormatado}.geojson`;
  ...
  // Effect para travar a Data Base da régua de tempo com base no timestep_000
  useEffect(() => {
    if (!fonteDados) return;

    fetch(`/data/${fonteDados}/nivel/geojson/timestep_000.geojson`)
          ...
          <Layer
            ...
            paint={{
              "fill-color": [
                ...
                ["linear"],
                ["get", "nivel"],
                -0.5,  "#440154", // Roxo escuro (Mínimo)
                -0.25, "#443983", // Roxo azulado
                0.0,   "#31688E", // Azul escuro
                0.25,  "#21908C", // Azul esverdeado / Ciano
                0.5,   "#20A387", // Ciano escuro
                0.75,  "#35B779", // Verde
                1.0,   "#4EA53B", // Verde claro / Amarelado
                1.25,  "#B4DE2C", // Amarelo limão
                1.5,   "#FDE725", // Amarelo puro
                1.75,  "#F8961E", // Laranja
                2.0,   "#DC2F02"  // Vermelho (Máximo)
              ],
              "fill-opacity": 0.9,
